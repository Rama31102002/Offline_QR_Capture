...
Users can choose **Export My Backup**. Administrators can choose **Export Full Backup**.

Full backups contain `backup.json` plus images under `images/`. Administrator full backups can also contain local user password verifiers, so store backup ZIP files securely.

Admin can use **Import Backup**. Existing record UUIDs are skipped instead of duplicated.
